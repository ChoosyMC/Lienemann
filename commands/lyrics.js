...
module.exports = {
  ...
  async execute (client, message, args) {
    ...
    .setDescription("Procurando pela letra da música..")
    .setColor("YELLOW")
    
    if(!args.length) {
      return message.channel.send("Você esqueceu de mencionar o nome da música.")
    }
     ...
     try {
          ...
          const lyrics = await songs[0].lyrics();
           ...
           if (lyrics.length > 4095) {
             msg.delete()
        return message.channel.send('As letras são muito longas para serem visualizadas.');
     }
